 ...
 * Date: Aug 23, 2024
 * Description: Templated structures for generic loop detection
 * License: See LICENSE.txt file at the top project folder
 * 
 * This file has been modified from the original version, attribution:
 * Original File: TemplatedLoopDetector.h
 * Original Author: Dorian Galvez-Lopez
 * Original Last Modified Date: May 7, 2020
 *
**/
...
#include <opencv2/calib3d.hpp>  //FIXME
...
enum GeometricalCheck
{
  ...
  GEOM_FLANN,
  /// Use exhaustive stereo checking
  GEOM_EXHAUSTIVE_STEREO,
  /// Use direct index with stereo
  GEOM_SDI,
...
protected:
  ...
  /**
   * Check if an old entry is spatially consistent with a given set of stereo
   * keypoints and descriptors by matching keypoints in all four images,
   * calculating the 3d point projections of those for a given pair and 
   * performing PnP matching with a single image of the other stereo pair.
   * Does not use direct index. 
   * @param old_keys keys of old entry
   * @param old_descriptors descriptors of old keys
   * @param cur_keys keys of current entry
   * @param cur_descriptors descriptors of cur keys
   */
  bool isGeometricallyConsistent_Exhaustive_Stereo(
    const std::vector<cv::KeyPoint> &old_keys1,
    const std::vector<TDescriptor> &old_descriptors1,
    const std::vector<cv::KeyPoint> &old_keys2,
    const std::vector<TDescriptor> &old_descriptors2,
    const std::vector<cv::KeyPoint> &cur_keys1,
...
protected:
  ...
  /// KeyPoints of stereo second images
  vector<vector<cv::KeyPoint> > m_image_keys2;
  ...
  (int nentries, int nkeys)
{
  ...
  if(sz < nentries)
  {
    ...
    m_image_keys2.resize(nentries);
    m_image_descriptors.resize(nentries);
    m_image_descriptors2.resize(nentries);
  }
  ...
  if(nkeys > 0)
  {
    for(int i = sz; i < nentries; ++i)
    {
      ...
      m_image_keys2[i].reserve(nkeys);
      m_image_descriptors[i].reserve(nkeys);
      m_image_descriptors2[i].reserve(nkeys);
    }
  }
...
/**
 * Detects a loop based on keypoints and descriptors of an image.
 * Will optionally take the keypoints and descriptors of a second image for
 * stereo loop detection. Its assumed that the keypoints for the stereo pair 
 * have already been matched and only their matching keys and descriptors are
 * given in matched order (i.e. keys[i] matches keys2[i] forall i and the same
 * for descriptors).
 *
 ...
 * @return boolean representing if the loop was detected or not
 */
template<class TDescriptor, class F>
  ...
  const std::vector<TDescriptor> &descriptors2)
{
  ...
  // We first find matches for the monocular or first stereo image (leaving
  // the geometric consistency or stereo geometric consistency for later).
  EntryId entry_id = m_database->size();
  match.query = entry_id;

  // Check if we have stereo information
  bool stereo = false;
  if (keys2.size() != 0) {
    stereo = true;
  }

  BowVector bowvec, bowvec2;
  FeatureVector featvec, featvec2;
  ...
  else
  {
    ...
    if(!qret.empty())
    {
      ...
      if(!m_params.use_nss || ns_factor >= m_params.min_nss_factor)
      {
        ...
        std::cout << "Normalized Score based on factor " << ns_factor << std::endl; 
        std::cout << "Removing scores lower than: " << m_params.alpha * ns_factor << std::endl;
  ...
  // std::cout << "m_image_keys.size(): " << m_image_keys.size() << std::endl;
  // std::cout << "entry_id: " << entry_id << std::endl;
  if(m_image_keys.size() == entry_id)
  {
    m_image_keys.push_back(keys);
    m_image_descriptors.push_back(descriptors);
    if (stereo) {
      m_image_keys2.push_back(keys2);
      m_image_descriptors2.push_back(descriptors2);
    }
  }
  else
  {
    ...
    if (stereo) {
      m_image_keys2[entry_id] = keys2;
      m_image_descriptors2[entry_id] = descriptors2;
    }
  }
  ...
  const std::vector<TDescriptor> &cur_descriptors) const
{
  ...
  std::cout << "[Exhaustive] found matches for " << (int)i_old.size() 
    << " keypoints" << std::endl;
  if((int)i_old.size() >= m_params.min_Fpoints)
  {
    // add matches to the vectors to calculate the fundamental matrix
...
// Stereo version of Exhaustive
// Its assumed that keys and descriptors of a stereo image pair have been
// matched beforehand and only their matches are passed here in order.
// i.e.: keys1[m] and keys2[m] are stereo matches for all m.
template<class TDescriptor, class F>
bool TemplatedLoopDetector<TDescriptor, F>::
  isGeometricallyConsistent_Exhaustive_Stereo(
    const std::vector<cv::KeyPoint> &old_keys1,
    const std::vector<TDescriptor> &old_descriptors1,
    const std::vector<cv::KeyPoint> &old_keys2,
    const std::vector<TDescriptor> &old_descriptors2,
    ...
    cv::Vec3d& transform) const
{
  ...
  // satisfy all 4 constraints (l1/r1 & l2/r2 & l1/l2 & r1/r2)
  std::vector<unsigned int> i_old1, i_cur1, i_old2, i_cur2;
  std::vector<unsigned int> i_all_old, i_all_cur;
  i_all_old.resize(old_keys1.size());
  i_all_cur.resize(cur_keys1.size());
  std::iota(i_all_old.begin(), i_all_old.begin() + old_keys1.size(), 0);
  std::iota(i_all_cur.begin(), i_all_cur.begin() + cur_keys1.size(), 0);
      ...
      ((int)i_old2.size() >= m_params.min_Fpoints))
  {
    ...
    // FIXME
    if (!(i_old.size() >= m_params.min_Fpoints))
    {
      return false;
    }

    // calculate the stereo triangulation of the first stereo pair
    std::vector<cv::KeyPoint> left_matches, right_matches;
    std::vector<cv::KeyPoint> cur_matches, cur_matches2;
    std::vector<unsigned int>::const_iterator oit, cit;
    oit = i_old.begin();
    cit = i_cur.begin();
    for (; oit != i_old.end(); ++oit, ++cit)
    {
      left_matches.push_back(old_keys1[*oit]);
      right_matches.push_back(old_keys2[*oit]);
      cur_matches.push_back(cur_keys1[*cit]);
    }

    // Prepare the data for the interface of triangulatePoints
    // FIXME: Looks repetitive, get it out of here
    cv::Mat left_points(2, left_matches.size(), CV_32F);
    for (size_t col = 0; col < left_matches.size(); col++) {
        left_points.at<float>(0, col) = left_matches[col].pt.x;
        left_points.at<float>(1, col) = left_matches[col].pt.y;
    }
    cv::Mat right_points(2, right_matches.size(), CV_32F);
    for (size_t col = 0; col < right_matches.size(); col++) {
        right_points.at<float>(0, col) = right_matches[col].pt.x;
        right_points.at<float>(1, col) = right_matches[col].pt.y;
    }
    cv::Mat cur_points(2, cur_matches.size(), CV_32F);
    for (size_t col = 0; col < cur_matches.size(); col++) {
        cur_points.at<float>(0, col) = cur_matches[col].pt.x;
        cur_points.at<float>(1, col) = cur_matches[col].pt.y;
    }
    cv::transpose(cur_points, cur_points);  // to Nx2
    std::vector<cv::Mat> stereo_points = {left_points, right_points};
    std::vector<cv::Mat> stereo_projection_matrices = 
      {m_params.stereo_params.left_projection, 
       m_params.stereo_params.right_projection};
    ...
    cv::transpose(stereo_3d_points, stereo_3d_points);  // to Nx3

    // Filter based on distance to keep only 3D points that agree with the
    // stereo camera's baseline threshold.
    ...
    for (int row = 0; row < stereo_3d_points.rows; row++) {
          ...
          (dist < this->m_params.far_distance))
      {
        ...
        good_3d_points.push_back(stereo_3d_points.row(row));
        good_cur_points.push_back(cur_points.row(row));
      }
    }
